  ...
  private void processData(
    ...
    try {
      ...
      if (!bizEvents.isEmpty()) {
        ...
        /*
        long count = bizEvents.stream().filter(e -> e.getId().equals("a5bc7464-e416-4955-8b72-b275467f50d1")).count();
        log.info("Quanti eventi con questo ID? " + count);
        
        for (BizEventEntity event : bizEvents) {
            if (event.getId().equals("a5bc7464-e416-4955-8b72-b275467f50d1")) {
                log.info("Evento {}, transferList size: {}", event.getId(), event.getTransferList().size());
                for (int i = 0; i < event.getTransferList().size(); i++) {
                    Transfer t = event.getTransferList().get(i);
                    log.info("Transfer idx={} idTransfer={} MBDAttachment='{}'", i, t.getIdTransfer(), t.getMBDAttachment());
                }
            }
        }*/
        ...
        List<String> mbdAttachments = bizEvents.stream()
                ...
                .filter(t -> StringUtils.isNotBlank(t.getMBDAttachment()))
                .map(Transfer::getMBDAttachment)
                .toList();
        
        log.info("mbdAttachments.size(): {}", mbdAttachments.size());*/
        ...
        //log.info("mbdAttachments.size() (deduplicated): {}", mbdAttachments.size());
